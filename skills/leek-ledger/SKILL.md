...
- Account/stock total P/L must reconcile to closed stock/option realized P/L plus dividends plus pure open-position floating P/L. Period stock payloads should expose trade realized P/L and dividend net separately: `nativeRealizedPnl` is trade realized, `nativeDividendPnl` is dividends/taxes, and `nativePnl` is the total. The current holdings table may roll partial sells, closed option income, and dividends into adjusted `all_in_cost`, but only for events on or after the current holding cycle start. If a symbol was fully cleared and later reopened, pre-clear realized P/L must stay in realized/total P/L and must not enter the new holding row's `float_pnl`; current holding summary return rates should follow the active holding cycle instead of lifetime `capital_raw`. When auditing, check closed-row formulas first, then compare each currency's latest return-curve point against `stock_summary.total_pnl` totals, allowing only tiny rounding/fx refresh drift.
