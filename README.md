...
刷新看板时会优先用公开行情源批量更新现股和期权价格：现股优先走东方财富批量行情，腾讯 / Yahoo 作为公开兜底；港股期权会尝试走 HKEX 延迟 Bid/Ask 中点，Futu OpenD 仅作为最后兜底。未平仓 covered call / cash-secured put 会展示现价、浮动盈亏和占用本金；取不到期权行情时会显示 `-`，不会阻塞页面生成。
