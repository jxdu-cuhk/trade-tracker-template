...
网页顶部按业务分成 `持仓 / 收益 / 复盘 / 明细 / 全部`，每页会组合相关区块；“刷新”和“统一口径币种”在同一条操作区，刷新进度默认是轻量状态条，只有刷新中或报错时展开步骤。

显示数据接口和当前口径来源整理在 `.trade-tracker/DATA_INTERFACES.md`。当前已新增统一展示层 `display_payload.py`，持仓总计、分币种折算、最新日浮盈、已实现交易列表和已实现日汇总会先在那里算好，再给页面区块复用。
