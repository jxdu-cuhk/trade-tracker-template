...
这个公开版本只保留可运行框架和空白工作簿，不包含任何真实交易记录、券商导出、标的缓存或个人数据。
...
如果本机运行了富途 OpenD，刷新看板时会优先用富途行情更新现股价格，并尝试匹配未平仓期权链，展示 covered call / cash-secured put 的现价、浮动盈亏和占用本金；取不到期权行情时会显示 `-`，不会阻塞页面生成。
