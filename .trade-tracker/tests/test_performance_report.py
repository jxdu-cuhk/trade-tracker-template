...
class PerformanceReportTests(unittest.TestCase):
    def test_insert_performance_report_after_return_curve(self):
        ...
        self.assertIn("optionalNumber", updated)
        self.assertIn("Number.isFinite(stockMetricValue(item))", updated)
        self.assertIn("stockDisplayName", updated)
        self.assertIn("absPnl", updated)
        self.assertIn("row.dataset.year || ''", updated)
        self.assertIn("盈利个股", updated)
        self.assertIn("reportingCurrency", updated)
        self.assertIn("trade-tracker-reporting-currency-change", updated)
